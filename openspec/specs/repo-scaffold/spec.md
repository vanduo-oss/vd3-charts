...
publish surface for `@vanduo-oss/vd3-charts`.
...
(`dist/vd3-charts.css`), and `./package.json`. It MUST also set `main`,
...
- **WHEN** a consumer resolves `@vanduo-oss/vd3-charts`
...
- **WHEN** a consumer resolves `@vanduo-oss/vd3-charts/css`
- **THEN** it resolves to `./dist/vd3-charts.css`

#### Scenario: vue is a required peer

- **GIVEN** a consumer project without `vue` installed
- **WHEN** the consumer installs `@vanduo-oss/vd3-charts` with strict peer
  enforcement
...
and copy `src/styles.css` to `dist/vd3-charts.css`. Builds SHALL be bundled,
...
`@vanduo-oss/vd3-charts` via package self-reference so the built declarations
...
suite SHALL load the built `dist/index.js` and assert real rendering for bar,
line, area, donut, and pie.
